...
def function_runner(func, solution):
    # --- Penalize shallow trees ---
    if solution.get_depth() < 3:
        for node in solution.L:
            if not node.is_feature:
                node.count = 1
        return solution, 1e12   # very bad score

    # otherwise run normally
    logger.debug("trying to run {} for {}".format(func, solution))
    results = func(solution)
    logger.debug("succesfully ran {} with {}: {}".format(func, solution,results))
    return solution, results
